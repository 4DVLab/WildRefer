...
class BeaUTyDETR(nn.Module):
    ...
    def __init__(self, num_class=50,
        ...
        if self.head_fuse:
            self.point_image_fusion = nn.ModuleList()
            for _ in range(3):
                self.point_image_fusion.append(PointImageFusionLayer(
                                    d_model, n_heads=8, dim_feedforward=256,
                                    dropout=0.1, activation="relu",
                                    self_position_embedding=self_position_embedding, head_fuse=self.head_fuse
                                ))
    ...
    def forward(self, inputs):
        ...
        if self.head_fuse:
            for i in range(len(self.point_image_fusion)):
                points_features = self.point_image_fusion[i](
                ...
                points_features = points_features.transpose(1, 2).contiguous()

...
SRC_PATH = "src/LifeRefer"

class LifeReferDataset(Dataset):
    def __init__(self, args, split="train") -> None:
        super().__init__()
        self.args = args
        if split == "train":
            self.dataset = json.load(open("data/liferefer_train.json"))
        else:
            self.dataset = json.load(open("data/liferefer_test.json"))

        self.find_previous = json.load(open(os.path.join(SRC_PATH, "find_previous_liferefer.json")))
